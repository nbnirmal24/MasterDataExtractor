    ...
    class homebargains
    {
        ...
        public static List<string> category(List<string> hulk)
        {
            ...
            jsk:
               ...
               if (str.IndexOf("ProductLink\" href=\"") > 0)
                {
                    pattern = @"ProductLink"" href=[^>]*?""(.*?)"">";
                    ...
                    foreach (Match match in matches)
                    {
                        ...
                        if (!bd.Contains("http"))
                        {
                            bd = "http://www.homebargains.co.uk" + bd;
                        }
                    ...
                    {
                        string dc = str.Substring((str.IndexOf(">></a>") - 90), str.IndexOf(">></a>") - (str.IndexOf(">></a>") - 90)) + ">></a>";
                        string next = "";
                        if (dc != "")
                        {
                            ...
                            if (next != "")
                            {
                                ...
                                {
                                    next = "http://www.homebargains.co.uk" + next;
                                }
                            }
                        }
                        str = "";
        ...
        phirse:
            ...
            jsk:
                ...
                try
                {
                    prdid = getsubstring("post\" action=\"", "-", str);
                ...
                try
                {
                    prodtit = getsubstring("<div id=\"productheader\">", "</div>", str);
                    prodtit = Regex.Replace(prodtit, @" ?\<.*?\>", string.Empty);
                    prodtit = Regex.Replace(prodtit, @"\s+", " ");
                }
                catch { }
                string price = "";
                string rawe = getsubstring("<div id=\"product\">", "<div class", str);
                try
                {
                    price = getsubstring("product-price\">", "</div>", str).Replace("&pound;", "£").Replace("Our price:","");
                ...
                prdcategry = getsubstring("<div id=\"rightheader1b\">", "<div class=\"clear\">", str).Replace("<img src", "|<img src").Replace("&#160;","");
                ...
                string deleveryinfo = "";
                string stockstatus = "";
                if (str.IndexOf("tures:") > 0)
                {
                    summry = getsubstring("<div id=\"productheader\">", "tures:", str);
                    if (summry != "")
                    {
                        des = getsubstring("</div>", "Fea", summry);
                        des = Regex.Replace(des, @" ?\<.*?\>", string.Empty);
                        des = Regex.Replace(des, @"\s+", " ");
                      
                    }
                    features = getsubstring(">Features:", "</ul>", str).Replace("</li>", "|");
                    features = Regex.Replace(features, @" ?\<.*?\>", string.Empty);
                    features = Regex.Replace(features, @"\s+", " ");
                }
                else
                {
                    summry = getsubstring("<div id=\"productheader\">", "v class=", str);
                    if (summry != "")
                    {
                        des = getsubstring("</div>", "<di", summry);
                        des = Regex.Replace(des, @" ?\<.*?\>", string.Empty);
                        des = Regex.Replace(des, @"\s+", " ");
                    }
              ...
              //  string tempstock = getsubstring("stockAvailabilityType = \"", "\";", str);
                if (str.IndexOf("value=\"BUY\" onclick=\"")>0)
                {
                ...
                string img = "";
                string imgraw = getsubstring("defaultImage\"", "</a>", str);
                if (imgraw != "")
                {
                    img = getsubstring("src=\"", "\" alt", imgraw).Replace("Size=300", "Size=1000").Replace("Size=100", "Size=1000").Replace("Size=10000", "Size=1000");
                }
                string altimg = "";
                List<string> imga = new List<string>();
                pattern = @"alt=""Product imag[^>]*?e(.*?)>";
                imga.AddRange(matchkar(str, pattern));
                if (imga.Count >= 1)
                {
                    foreach (string accesss in imga)
                    {
                        string iii = getsubstring("Image('", "')", accesss).Replace("Size=300", "Size=1000").Replace("Size=100", "Size=1000").Replace("Size=10000", "Size=1000");
                        if (iii != "")
                        {
                            altimg = iii+"|" + altimg;
                        }
                ...
                string shipp = "";
                string variable = "";
                if (str.IndexOf("class=\"variantName\"") > 0)
                {
                    pattern = @"class=""variantName[^>]*?""(.*?)</div>";
                    List<string> mmaa = new List<string>();
                    mmaa.AddRange(matchkar(str, pattern));
                    foreach (string abcd in mmaa)
                    {
                        string varname = getsubstring("\">", "<", abcd);
                        if (varname != "")
                        {
                            variable = varname;
                        }
                        string imdx = getsubstring("changePicture(", ")", abcd).Replace("&#39;", "").Replace("Size=300", "Size=1000").Replace("Size=100", "Size=1000").Replace("Size=10000", "Size=1000");
                        if (imdx != "")
                        {
                            img = imdx;
                        }
                        if (abcd.IndexOf(">Sold out") > 0)
                        {
                            stockstatus = "out of stock";
                        }
                        string ppp = getsubstring("ProductPrice\"", "</span>", abcd);
                        if (ppp != "")
                        {
                            ppp = "<" + ppp;
                            ppp = Regex.Replace(ppp, @" ?\<.*?\>", string.Empty);
                            ppp = Regex.Replace(ppp, @"\s+", " ");
                            price = ppp;
                        }
                        if (price != "")
                        {
                            try
                            {
                                ...
                                pss:
                                    ...
                                    if (str.Contains("did not match any products."))
                                    {
                                        ...
                                        qvcp.Rows.Add(prdid, prdcategry, prodtit, variable, img, altimg, price, stockstatus, des, features, rhon, prodtit, asinp, rankp, categoryp, numselp, pricep, shipp);
